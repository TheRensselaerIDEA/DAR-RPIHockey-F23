...
subtitle: "DAR Assignment 2 - Hockey (Fall 2023)"
author: "Amy Enyenihi"
date: "`r Sys.Date()`"
output:
...
rF <- randomForest(outcomes.goal ~. , data = hockeyTrain)

print(rF)   #results and confusion matrix of the random forest

plot(rF)   #visualizing the error rate. the error rate stabilizes with more trees

pred <- predict(rF, newdata = hockeyTest)
pred_matrix <- confusionMatrix(table(pred, hockeyTest$outcomes.goal))   #the prediction to compute the confusion matrix and see the accuracy score
pred_matrix
```
...
```{r}
rF_accuracy <- pred_matrix$overall[1]

#comparing balanced model 1 to the balanced accuracy of random forest
if (rF_accuracy < balancedAccuracyTest1) {
  print(paste0("Balanced accuracy decreased: Random Forest = ", rF_accuracy, " < Model1 = ", balancedAccuracyTest1))
} else if (rF_accuracy > balancedAccuracyTest1) {
  print(paste0("Balanced accuracy increased: Random Forest = ", rF_accuracy, " > Model1 = ", balancedAccuracyTest1))
} else {print("No  change...")}

#comparing balanced model 2 to the balanced accuracy of random forest
if (rF_accuracy < balancedAccuracyTest2) {
  print(paste0("Balanced accuracy decreased: Random Forest = ", rF_accuracy, " < Model2 = ", balancedAccuracyTest2))
} else if (rF_accuracy > balancedAccuracyTest2) {
  print(paste0("Balanced accuracy increased: Random Forest = ", rF_accuracy, " > Model2 = ", balancedAccuracyTest2))
} else {print("No  change...")}
```
...
```{r}
importance(rF)   #listing all the variables with their relative importance
varImpPlot(rF)   #plotting the list above
```

Discuss your results and what they mean. 
```{r}

#The graph produced plots the variables in increasing importance. The puck distance was deemed the most important feature because it was scored the highest number. 
