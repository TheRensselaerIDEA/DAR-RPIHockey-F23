...
subtitle: "zhangl38's DAR Assignment 2 (Fall 2023)"
author: "Liebin Zhang"
date: "`r Sys.Date()`"
output:
...
rf_model <- randomForest(outcomes.goal ~ ., data=hockeyTrain, importance=TRUE)
...
my_model_result<-predict(rf_model, hockeyTest, type='prob')

prob_goal <- my_model_result[, 2]

my_model_cm <- as.matrix(table(Actual = hockeyTest$outcomes.goal, Predicted = prob_goal>0.5))
...
X <- data.matrix(hockeyTrain[, -which(names(hockeyTrain) == "outcomes.goal")])
y <- hockeyTrain$outcomes.goal

# Apply LASSO
lasso_model <- cv.glmnet(X, y, alpha=1 ,family = "binomial")  # alpha=1 implies LASSO

# Getting the coefficients at the best lambda
...
```{r}
# insert your code here for creating assessing
#By my accuracy is 0.83333, my balance accuracy. However, due to insufficient data volume resulting in inaccurate results, we may need more data for analysis.
# By LASSO feature selection, this four features are the most important and goalieScreened has the greatest impact on goals.
ggplot(data = hockeyTrain, mapping = aes(x=puckAngle, y=puckDist,fill=outcomes.goal)) +
    geom_point(alpha = 0.8, aes(color = outcomes.goal))+
    ggtitle(" ")
#The result of my analysis is that the probability of hitting the target is highest when the distance is less than 250 and the angle is between 40-120 degrees.
ggplot(data = hockeyTrain, mapping = aes(x=posTime, y=goalieScreened,fill=outcomes.goal)) +
    geom_point(alpha = 0.8, aes(color = outcomes.goal))+
    ggtitle(" ")
#The shorter the postime(less than 20s) and the less goalkeeper interference, the higher the success rate of goals.
```
